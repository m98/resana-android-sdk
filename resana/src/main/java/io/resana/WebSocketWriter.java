...
class WebSocketWriter extends Handler {
    private static final String TAG = ResanaLog.TAG_PREF + "WebSocketWriter";
    ...
    private void notify(WebSocketMessage.Message message) {
        if (stopped) {
            ...
            ResanaLog.w(TAG, "resana Writer Tried To Notify Master In Closed State!");
            return;
        }
    ...
    private void sendClientHandshake(WebSocketMessage.ClientHandshake message) throws IOException {
        ...
        if (message.mHeaderList != null) {
            for (KeyValue pair : message.mHeaderList) {
                mBuffer.write(pair.getKey() + ":" + pair.getValue());
                mBuffer.crlf();
            }
        }
        mBuffer.crlf();
    ...
    public void handleMessage(Message msg) {
        ...
        try {
            ...
            mSocket.getOutputStream().flush(); // hojjat
            success = true;

        } catch (SocketException e) {

            ResanaLog.v(TAG, "run() : SocketException (" + e.toString() + ")");

            // wrap the exception and notify master
            notify(new WebSocketMessage.ConnectionLost());
        } catch (SSLException e) {
            ResanaLog.v(TAG, "run() : SSLException (" + e.toString() + ")");

            // wrap the exception and notify master
            notify(new WebSocketMessage.ConnectionLost());
        } catch (WebSocketException e) {
            ResanaLog.v(TAG, "run() : WebSocketException (" + e.toString() + ")");
            notify(new WebSocketMessage.ConnectionLost());
        } catch (IOException e) {
            ResanaLog.v(TAG, "run() : IOException (" + e.toString() + ")");
            notify(new WebSocketMessage.ConnectionLost());
        } catch (Exception e) {
            ResanaLog.e(TAG, "unExpected Exception! (handled)");
            // wrap the exception and notify master
        ...
        if (msg.obj instanceof WebSocketMessage.ResanaAckMessage) {
            WebSocketMessage.ResanaAckMessage ack = (WebSocketMessage.ResanaAckMessage) msg.obj;
            ack.sentSuccessfully = success;
            notify(ack);
        }
    }
