...
class WebSocketReader extends Thread {
    private static final String TAG = ResanaLog.TAG_PREF + "WebSocketReader";
    ...
    protected void notify(WebSocketMessage.Message message) {
        if (mState == STATE_CLOSED) {
            ...
            ResanaLog.w(TAG, "resana Reader Tried To Notify Master In Closed State!");
            return;
        }
    ...
    private Map<String, String> parseHttpHeaders(byte[] buffer) throws UnsupportedEncodingException {
        ...
        for (String line : lines) {
            if (line.length() > 0) {
                ...
                if (h.length == 2) {
                    headers.put(h[0], h[1]);
                    ResanaLog.w(TAG, String.format("'%s'='%s'", h[0], h[1]));
                }
            }
        }
    ...
    private Pair<Integer, String> parseHttpStatus() throws UnsupportedEncodingException {
        ...
        ResanaLog.w(TAG, String.format("Status: %d (%s)", statusCode, statusMessage));
        ...
        try {
            ...
            do {
                ...
                if (len > 0) {
                    ...
                    ResanaLog.v(TAG, "run() : ConnectionLost");
            ...
            ResanaLog.e(TAG, "(handled) unExpected Exception!");
            // wrap the exception and notify master
